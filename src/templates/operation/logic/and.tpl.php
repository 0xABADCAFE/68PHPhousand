...
<?php
    ...
    switch ($iSize) {
        case IOpcode::OP_SIZE_B:
?>
    $iValue = $this->oDataRegisters->iReg<?= $iDataReg ?> &= (ISize::MASK_INV_BYTE|$oEAMode->readByte());
    $this->iConditionRegister &= IRegister::CCR_CLEAR_CV;
    $this->updateNZByte($iValue);
<?php
            break;
        case IOpcode::OP_SIZE_W:
?>
    $iValue = $this->oDataRegisters->iReg<?= $iDataReg ?> &= (ISize::MASK_INV_WORD|$oEAMode->readWord());
    $this->iConditionRegister &= IRegister::CCR_CLEAR_CV;
    $this->updateNZWord($iValue);
<?php
            break;
